...
# This line helps resolve "RuntimeError: Tried to instantiate class '__path__._path'"
# often seen when PyTorch is used with Streamlit's file watcher.
torch.classes.__path__ = []

# --- Set up API Key with error handling ---
# API key is securely loaded from Streamlit's secrets.
# If running locally, ensure you have a .streamlit/secrets.toml file with api_key = "YOUR_API_KEY".
try:
...
# --- Sentiment Pipeline ---
# Use Streamlit's cache_resource to load the large sentiment analysis model only once
# across all user sessions and reruns, saving memory and load time.
@st.cache_resource
...
def get_comments_with_likes_filtered(video_id, max_comments=500, max_retries=5):
    ...
    st.info(f"Attempting to fetch comments for video ID: {video_id}") # Debugging
    ...
    for attempt in range(max_retries):
        try:
            ...
            while len(comments) < max_comments:
                st.write(f"Fetching page {len(comments) // 100 + 1} (Attempt {attempt + 1})...") # Debugging
